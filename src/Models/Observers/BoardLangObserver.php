...
namespace WalkerChiu\MorphBoard\Models\Observers;

class BoardLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.morph-board.boardLang')
                ::withTrashed()
        ...
        if (
            config('wk-morph-board.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-morph-board.lang_log')
            )
        ) {
